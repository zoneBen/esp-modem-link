...
// settled first, and the module has to be told rather than followed: with
// AT+IPR=0, its factory setting, ML307R-DL-MBRH0S01 stayed put while three
// commands at 921600 all timed out.
...
TEST(Ml307HalInitTest, AlignsTheModuleToTheConfiguredRate) {
  MockAtChannel channel;
...
TEST(Ml307HalInitTest, FindsTheModuleOnARaisedRateAfterOpeningAtTheDefault) {
  MockAtChannel channel;
...
TEST(Ml307HalInitTest, LeavesAMatchingRateAlone) {
  MockAtChannel channel;
...
TEST(Ml307HalInitTest, RetriesAProbeLostToTheRateSwitch) {
  MockAtChannel channel;
  channel.SetModuleBaudRate(115200);
  channel.SetBaudRate(921600);
  channel.ExpectCommand("AT+IPR?")
...
TEST(Ml307HalInitTest, DoesNotAskForARateTheModuleRejects) {
  MockAtChannel channel;
  channel.SetModuleBaudRate(115200);
  channel.SetBaudRate(921600);
  channel.ExpectCommand("AT+IPR?").Respond("+IPR: 115200\r\nOK\r\n");
...
TEST(Ml307HalInitTest, DoesNotMatchARateThatIsOnlyAPrefix) {
  MockAtChannel channel;
  channel.ExpectCommand("AT+IPR?").Respond("+IPR: 1152000\r\nOK\r\n");
  channel.ExpectCommand("AT+IPR=?")
...
// working. Refusing to initialise would leave the caller with no device at all.
TEST(Ml307HalInitTest, ReturnsToTheWorkingRateWhenTheNewOneIsNotConfirmed) {
  MockAtChannel channel;
  channel.SetBaudRate(921600);
  ...
  EXPECT_EQ(channel.GetBaudRate(), 115200);
  EXPECT_TRUE(channel.WasCommandSent("AT+IPR=115200"));
  // And the session is usable rather than abandoned.
  EXPECT_TRUE(channel.WasCommandSent("ATE0"));
}

// A module that never answers the query is not a device that cannot be used: the
// firmware may not implement AT+IPR?, or may sit on a rate this library does not
// know to probe. Initialisation carries on, and the channel is left where the
// caller configured it rather than on the last rate probed - a caller reading
// GetBaudRate() afterwards must not be told a rate the port was only passing
// through.
TEST(Ml307HalInitTest, LeavesTheChannelOnTheConfiguredRateWhenNothingAnswers) {
  MockAtChannel channel;
  channel.SetBaudRate(921600);
...
TEST(Ml307HalInitTest, InitialisesWhenTheModuleWillNotReportItsRate) {
  MockAtChannel channel;
  channel.FailCommand("AT+IPR?");
  channel.ExpectCommand("ATE0").Respond("OK\r\n");
  channel.ExpectCommand("AT+CFUN=1").Respond("OK\r\n");
